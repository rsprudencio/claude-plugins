...
class TestGetPostgresConfig:
    """Tests for get_postgres_config with URL and env overrides."""

    def test_defaults_from_config(self, mock_config):
        """Should return postgres_url from config."""
    ...
    def test_env_var_override(self, mock_config, monkeypatch):
        """POSTGRES_URL env var should take precedence over config file."""
        from tools.config import get_postgres_config

        monkeypatch.setenv("POSTGRES_URL", "postgresql://env:pass@envhost:5433/jarvis")
        cfg = get_postgres_config()
        assert cfg["url"] == "postgresql://env:pass@envhost:5433/jarvis"

    def test_missing_config_returns_default(self, mock_config):
        """When postgres_url is not configured, should return default."""
        from tools.config import get_postgres_config

        mock_config.set(memory={})  # No postgres_url key
        cfg = get_postgres_config()
        assert "url" in cfg
