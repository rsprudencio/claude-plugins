...
Verifies that:
- Content writes include user metadata when authenticated
...
class TestContentUserAttribution:
    """Content writes should include user metadata when authenticated."""

    def test_content_write_includes_user_metadata(self, mock_config):
        ...
        try:
            ...
            doc = content_read(result["id"])
            assert doc["found"] is True
            assert doc["metadata"]["user"] == "raph"
        finally:
            current_user.reset(token)

    def test_content_write_anonymous_has_no_user_field(self, mock_config):
    ...
    def test_content_write_different_users(self, mock_config):
        ...
        doc1 = content_read(r1["id"])
        doc2 = content_read(r2["id"])
...
class TestOwnershipChecks:
    ...
    def test_delete_other_user_content_fails(self, mock_config):
        """User cannot delete another user's content."""
        from tools.content import content_write
        from tools.remove import remove
