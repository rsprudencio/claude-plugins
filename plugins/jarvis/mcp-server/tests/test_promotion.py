...
class TestCheckPromotionCriteria:
    ...
    def test_custom_thresholds(self, mock_config):
        ...
        mock_config.set(
            promotion={"importance_threshold": 0.95, "retrieval_count_threshold": 10}
        )

        # Test with default high importance (0.90) - should not trigger now
        metadata = {"importance_score": "0.90", "retrieval_count": "5"}
        result = check_promotion_criteria(metadata)
        assert not result["should_promote"]
...
class TestPromote:
    ...
    def test_promote_pattern(self, mock_config):
        ...
        assert result["success"]
        assert (
            "patterns_promoted" in result["promoted_path"]
            or "pattern" in result["promoted_path"]
        )
    ...
    def test_promote_summary(self, mock_config):
        ...
        assert result["success"]
        assert (
            "summaries_promoted" in result["promoted_path"]
            or "summar" in result["promoted_path"]
        )
