...
def check_promotion_criteria(metadata: dict) -> dict:
    ...
    if importance_score >= config["importance_threshold"]:
        criteria_met.append(
            f"importance {importance_score:.2f} >= {config['importance_threshold']}"
        )

    # Criterion 2: High retrieval count
    if retrieval_count > config["retrieval_count_threshold"]:
        criteria_met.append(
            f"retrievals {retrieval_count:.2f} > {config['retrieval_count_threshold']}"
        )
    ...
    if created_at:
        try:
            ...
            if (
                days_old >= config["age_importance_days"]
                and importance_score >= config["age_importance_score"]
            ):
                criteria_met.append(
...
def promote(doc_id: str) -> dict:
    ...
    try:
        ...
        relative_path = os.path.relpath(
            os.path.join(promotion_dir, filename), vault_path
        )
        ...
        tags = [
            tag.strip() for tag in metadata.get("tags", "").split(",") if tag.strip()
        ]
