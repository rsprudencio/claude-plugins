...
def find_conflict_candidates(
    ...
    """Query ChromaDB for Tier 2 entries that may conflict with *content*.

    Returns candidates with high embedding similarity but low word Jaccard
    (same topic, different wording → likely contradiction).
    """
    ...
    for cid, distance, doc_content, metadata in zip(ids, distances, documents, metadatas):
        ...
        old_words = _tokenize(doc_content or "")
...
def mark_superseded(old_doc_id: str, new_doc_id: str) -> bool:
    ...
    try:
        collection = _get_collection()
        result = collection.get(ids=[old_doc_id], include=["documents", "metadatas"])
        if not result["ids"]:
            return False
