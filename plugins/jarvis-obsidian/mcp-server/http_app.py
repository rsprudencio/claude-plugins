"""
Streamable HTTP transport for Jarvis Obsidian MCP Server.

Thin ASGI wrapper around the existing stdio-based server.py,
enabling Docker deployment via uvicorn.

Usage:
    uvicorn http_app:app --host 0.0.0.0 --port 8744
"""
...
def _get_version():
    ...
    try:
        ...
        return version("jarvis-obsidian")
    except Exception:
...
async def health_response(scope, receive, send):
    ...
    await _json_response(
        send, {"status": "ok", "server": "jarvis-obsidian", "version": _VERSION}
    )
