...
logger = logging.getLogger("jarvis-obsidian.git_ops")
...
def parse_last_commit() -> dict:
    ...
    if not hash_success:
        return {"success": False, "error": "Failed to get commit hash"}
    ...
    if not subject_success:
        return {"success": False, "error": "Failed to get commit subject"}
...
def query_history(
    ...
    # Use ASCII Record Separator (0x1E) — a control character that never appears in commit messages
    sep = "\x1e"
    args = ["log", f"--max-count={limit}", f"--format=%H%x1e%s%x1e%ai"]
    ...
    for line in result.get("stdout", "").strip().split("\n"):
        ...
        if operation != "all":
            ...
            op_patterns = {
                "create": r"Jarvis CREATE:",
                "edit": r"Jarvis EDIT:",
                "delete": r"Jarvis DELETE:",
                "move": r"Jarvis MOVE:",
                "user": r"User updates:",
            }
            pattern = op_patterns.get(operation)
            if pattern and not re.search(pattern, subject, re.IGNORECASE):
                continue
    ...
    return {"success": True, "operations": operations, "count": len(operations)}
...
def rollback_commit(commit_hash: str) -> dict:
    ...
    return {"success": True, "revert_hash": revert_hash, "reverted_commit": commit_hash}
...
def rewrite_commit_messages(
    count: int = 1, patterns: Optional[list[str]] = None
) -> dict:
    ...
    old_success, old_result = run_git_command(["log", f"-{count}", "--format=%H"])
    ...
    new_success, new_result = run_git_command(["log", f"-{count}", "--format=%H"])
