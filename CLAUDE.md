...
- **1.22.0** - Native Todoist API: add dedicated MCP server to jarvis-todoist plugin wrapping Todoist REST API v2 directly (local stdio, no session drops), eliminate external HTTP MCP dependency (ai.todoist.net/mcp), 9 tools (find_tasks, find_tasks_by_date, add_tasks, complete_tasks, update_tasks, delete_object, user_info, find_projects, add_projects), sync httpx client with lazy singleton + cached inbox resolution, tool name migration across 5 files (agent, 3 skills, system-prompt), `todoist.api_token` config key in defaults/config.json, 63 new tests (1052 total across both servers). jarvis-todoist bumped to v1.4.0.
